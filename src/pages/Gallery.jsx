...
import { Plus, Search, Trash2, MoreHorizontal, User, ImageIcon, X, Upload, Loader2, Star, Medal, Zap, Crown, Sparkles, Rocket } from 'lucide-react'
...
export function Gallery() {
  ...
  const handleRename = async () => {
    ...
    try {
      ...
      if (error) throw error
      setPaintings(paintings.map(p => p.id === editingPainting.id ? { ...p, title: newTitle.trim() } : p))
  ...
  return (
    <div className="space-y-8 md:space-y-12 pb-12">
        ...
        {filteredPaintings.map((painting) => (
          <div key={painting.id} className="glass-card group relative overflow-hidden flex flex-col h-full hover:-translate-y-2 transition-all duration-500 border-white/5 hover:border-purple-500/30 shadow-2xl">
            <div className="aspect-[4/3] overflow-hidden relative">
              ...
              />
              <div className="absolute top-6 left-0 right-0 px-6 flex items-center justify-between z-10 pointer-events-none">
                 <div className="px-4 py-2 bg-black/70 backdrop-blur-xl border border-white/10 rounded-2xl shadow-2xl flex items-center gap-2 transition-all duration-500">
                    <div className={`w-2 h-2 rounded-full ${painting.is_ai_generated ? 'bg-purple-500 shadow-[0_0_10px_rgba(168,85,247,0.5)] animate-pulse' : 'bg-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.5)]'}`}></div>
                    <span className="text-[10px] font-black text-white uppercase tracking-widest whitespace-nowrap">
                      {painting.is_ai_generated ? t('ai_gen') : t('handmade')}
                    </span>
                 </div>
                 {painting.is_finished && (
                   <div className="px-4 py-2 bg-emerald-500/95 backdrop-blur-xl rounded-2xl flex items-center gap-2 shadow-2xl border border-emerald-400/30 transition-all duration-500">
                      <Star className="w-3 h-3 text-white fill-white" />
                      <span className="text-[10px] font-black text-white uppercase tracking-widest whitespace-nowrap">{t('masterpiece')}</span>
                   </div>
                 )}
              </div>
              <div className="absolute top-4 right-4 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all flex gap-3 z-10">
                 <button 
                  onClick={() => { setEditingPainting(painting); setNewTitle(painting.title); }}
                  className="w-12 h-12 bg-black/60 backdrop-blur-xl border border-white/10 rounded-2xl text-white flex items-center justify-center hover:bg-purple-600 transition-all shadow-xl"
                  title="Rename"
                 >
                    <MoreHorizontal className="w-6 h-6" />
                 </button>
                 <button 
                  onClick={() => { setReplacingId(painting.id); replaceInputRef.current?.click(); }}
                  className="w-12 h-12 bg-black/60 backdrop-blur-xl border border-white/10 rounded-2xl text-white flex items-center justify-center hover:bg-indigo-600 transition-all shadow-xl"
                  title="Replace Image"
                 >
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 bg-gradient-to-t from-black via-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-end">
                <div className="grid grid-cols-3 gap-3">
                  <button 
                    onClick={() => setSelectedImage(painting)}
                    className="aspect-square bg-white text-purple-900 rounded-2xl flex flex-col items-center justify-center gap-1 hover:bg-purple-50 transition-all shadow-xl active:scale-95"
                  >
                    <User className="w-5 h-5" />
                    <span className="text-[8px] font-black uppercase tracking-tighter">View</span>
                  </button>
                  <button 
                    onClick={() => toggleFinished(painting)}
                    className={`aspect-square rounded-2xl transition-all border flex flex-col items-center justify-center gap-1 shadow-xl active:scale-95 ${painting.is_finished ? 'bg-emerald-500/20 border-emerald-500/30 text-emerald-500' : 'bg-purple-600 border-purple-500 text-white hover:bg-purple-500'}`}
                  >
                    {painting.is_finished ? <Loader2 className="w-5 h-5" /> : <Star className="w-5 h-5" />}
                    <span className="text-[8px] font-black uppercase tracking-tighter">{painting.is_finished ? t('edit') : t('finish')}</span>
                  </button>
                  <button 
                    onClick={(e) => { e.stopPropagation(); handleDelete(painting.id); }}
                    className="aspect-square bg-red-500/20 backdrop-blur-xl border border-red-500/30 rounded-2xl text-red-500 hover:bg-red-500 hover:text-white transition-all shadow-xl active:scale-95 flex flex-col items-center justify-center gap-1"
                  >
      ...
      {editingPainting && (
        <div className="fixed inset-0 z-[120] bg-black/80 backdrop-blur-xl flex items-center justify-center p-6 animate-in zoom-in duration-300">
           <div className="glass-card p-10 w-full max-w-md space-y-6">
              <h3 className="text-2xl font-black text-white uppercase tracking-tighter">Rename Work</h3>
              <input 
                type="text" 
                value={newTitle}
                onChange={(e) => setNewTitle(e.target.value)}
                className="w-full h-14 px-6 bg-white/[0.03] border border-white/5 rounded-2xl focus:outline-none focus:border-purple-500 text-white font-bold"
              />
      ...
      {selectedImage && (
        <div className="fixed inset-0 z-[100] bg-black/95 backdrop-blur-3xl flex items-center justify-center p-10 animate-in fade-in duration-300">
           <button 
            onClick={() => setSelectedImage(null)}
            className="absolute top-10 right-10 w-14 h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center text-white hover:bg-red-500 transition-all z-[110]"
           >
              <X className="w-8 h-8" />
           </button>
           <div className="relative w-full h-full flex items-center justify-center">
              ...
              <div className="absolute bottom-0 left-0 right-0 p-10 bg-gradient-to-t from-black to-transparent">
                 <h2 className="text-4xl font-black text-white uppercase tracking-tighter mb-2">{selectedImage.title}</h2>
                 <p className="text-gray-400 font-medium">{selectedImage.description}</p>
              </div>
            </div>
        </div>
      )}
