...
export function ReportsQueue({ onOpenPost, onViewProfile }) {
  ...
  const resolve = async (report, newStatus) => {
    ...
    if (ok) {
        await logAdminAction('update_report_status', 'report', report.id, { oldStatus: report.status, newStatus })
    }
    setBusyId(null)
    if (!ok) return
  ...
  const deletePost = async (report) => {
    ...
    if (ok) {
        await logAdminAction('delete_post', 'post', report.target_id, { reportId: report.id })
  ...
  const deleteComment = async (report) => {
    ...
    if (ok) {
        await logAdminAction('delete_comment', 'comment', report.target_id, { reportId: report.id })
        await updateReportStatus(report.id, 'reviewed')
    }
    setBusyId(null)
    if (ok && status !== 'all') removeLocal(report.id)
  ...
  const toggleBan = async (report) => {
    ...
    if (ok) {
        await logAdminAction(currentlyBanned ? 'unban_user' : 'ban_user', 'user', report.target_id, { reportId: report.id })
    }
    setBusyId(null)
