-- Migration admin_users_style_fixes.sql
-- Объединяет доработки кастомизации профиля (nickname_color, avatar_frame, finished_work_count)
-- с реферальными полями (referral_code, referrer_host), чтобы функции не перезаписывали
-- друг друга. Также обогащает функции подписок и реферальных пользователей этими стилями.
...
BEGIN
    ...
    SELECT coalesce(jsonb_agg(row_to_json(t)), '[]'::jsonb) INTO v_rows FROM (
        SELECT
            ...
            p.nickname_color as user_nickname_color,
            p.avatar_url as user_avatar,
            p.avatar_frame as user_avatar_frame,
...
-- Переприсвоение привилегий на всякий случай
